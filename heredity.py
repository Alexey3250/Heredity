...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person, info in people.items():
        ...
        # If the person has no parents, gene probability is unconditional
        if mother is None and father is None:
            gene_prob = PROBS['gene'][genes]
        else:
            passing = {mother: 0, father: 0}

            # For each parent, calculate the probability of passing a gene
            for parent in [mother, father]:
                passing[parent] = (
                    # If parent has 2 genes, they can only pass the gene unless it mutates
                    1 - PROBS["mutation"] if parent in two_genes else
                    # If parent has 1 gene, 50-50 chance of passing the gene
                    0.5 if parent in one_gene else
                    # If parent has 0 genes, they can only pass the gene if it mutates
                    PROBS["mutation"]
                )
        ...
        # Combine gene and trait probabilities
